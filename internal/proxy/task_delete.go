...
func (dr *deleteRunner) Init(ctx context.Context) error {
	...
	if err != nil {
		metrics.ProxyParseExpressionLatency.WithLabelValues(paramtable.GetStringNodeID(), "delete", metrics.FailLabel).Observe(float64(time.Since(start).Milliseconds()))
		return merr.WrapErrAsInputError(merr.WrapErrParameterInvalidMsg("failed to create delete plan: %v", err))
	}
	metrics.ProxyParseExpressionLatency.WithLabelValues(paramtable.GetStringNodeID(), "delete", metrics.SuccessLabel).Observe(float64(time.Since(start).Milliseconds()))
