...
func runScannBenchmark(ctx context.Context, c client.Client, vectors [][]float32, gt [][]int64, nprobe int, reorderK int) {
	fmt.Printf("\n=== SCANN nprobe=%d, reorder_k=%d ===\n", nprobe, reorderK)

	sp, _ := entity.NewIndexSCANNSearchParam(nprobe, reorderK)
...
func main() {
	...
	sp, _ := entity.NewIndexSCANNSearchParam(32, 200)
	for i := 0; i < 10; i++ {
	...
	for _, cfg := range configs {
		runScannBenchmark(ctx, c, vectors, gt, cfg.nprobe, cfg.reorderK)
	}
	fmt.Println("\n========== Benchmark Complete ==========")
}
